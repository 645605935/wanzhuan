...
class Image {
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        ...
        $_string=new \Org\Util\String();

        $randval = $_string::randString($length, $mode);
    ...
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='simhei.ttf', $verifyName='verify') {
        ...
        $code = $_string::randString($length, 4);
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $codex = $_string::msubstr($code, $i, 1);
    ...
    /**
    *上传图片生成略显图 裁剪成固定的高和宽
    * @param $files 可以是$_FILES | string 
    * @param $path 不带STATIC_PATH的路径 
    * @param $function thumb按比例裁剪 thumb2正方形裁剪
    * @param $info 图片信息 生成图片前缀,宽,高|第二张图前缀,宽，高 
    * @param $original 是保留原图 1是0否 默认为1  如果要原图 返回数组最后一个是原图
    * @return array
    */
    static function upload($files,$path,$function,$phone=0,$info='thumb_,120,120|bmid_,440,440',$original=1){
        if($phone==1){//判断是不是手机上传
            $ex = explode(",",$files);
            $filter=explode('/', trim($ex[0],';base64'));
            $imgpath=STATIC_PATH.$path.time().rand(100,999999).'.'.$filter[1];
            $filepath['url'] = file_put_contents($imgpath,  base64_decode($ex[1])) ?  $imgpath : false;
            $ii = self::getImageInfo($filepath['url']);
        }elseif(strstr($files,STATIC_PATH.'temp/')){//这个为uploady 那个flash上传的 判断 必须存在temp里哈
            $filepath['url']=$files;
            $ii = self::getImageInfo($filepath['url']);
        }elseif(strstr($files,STATIC_PATH)){//这个只要有字符串里有./Uploads/他就会去拿图片了
            $filepath['url']=$files;
            $ii = self::getImageInfo($filepath['url']);
        }elseif(strstr($files,'http://')){
        	...
        	$ii['type'] = $end = end(explode(".",$files));
        	$imgpath=STATIC_PATH.$path.'/'.time().rand(100,99999999).'.'.$end;
        	$filepath['url'] = file_put_contents($imgpath, file_get_contents($files,0,$ctx) ) ?  $imgpath : false;
        }else{//否则是files上传方法
            $filepath=self::filesMoveToTemp($files,array('jpg','png','gif','jpeg'));
            $ii = self::getImageInfo($filepath['url']);
        }
        $file=end(explode('/', $filepath['url'])); 
        ...
        if($function){
             foreach ($info_list as $v) {
                ...
                if( self::$function( $filepath['url'] ,  STATIC_PATH.$thumbnailPath ,$ii['type'], $pic_info[1], $pic_info[2])){
     ...
     **/
     static function filesMoveToTemp($files,$uptypes=array('jpg','png','gif','jpeg')){
    ...
    //获得编辑器里的第一张图并生成略显图
    public function firstImageThumb($text,$path,$function='thumb',$phone=0,$info='thumb_,120,120|bmid_,440,440',$original=1){
        $text=htmlspecialchars(stripslashes($text));
        preg_match('/'.preg_quote(STATIC_PATH,'/').'(.*)(?<=\.png|\.jpg|\.jpeg|\.gif)/isU', $text, $matches);
        if(!empty($matches)){
            return $this->upload($matches[0],dirname($path),$function,$phone,$info,$original);
        }else{
            return false;
        }
    }
